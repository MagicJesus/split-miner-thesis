...
class DFG:
    def __init__(self, log, concurrency_threshold, filtering_percentile_threshold, artificial_log=False):
        self.log = log
        self.source = None
        self.sink = None
        ...
        self.prepare_log(artificial_log)

    def prepare_log(self, artificial_log):
        traces = []
        if not artificial_log:
            for trace in self.log:
                event_list = []
                for event in trace:
                    event_list.append(event['concept:name'])
                event_list.append('End')
                event_list.insert(0, 'Start')
                traces.append(event_list)
        else:
            traces = self.log
            self.source = DFGNode('a')
            self.sink = DFGNode('h')
        ...
        # print("CONCURRENCY:", self.concurrency_relations)
        # print("INFREQUENT:", self.infrequent_behavior)
        # print("AFTER PRUNE:")
    ...
    def filtration(self):
        ...
        # print("THRESHOLD: ", filtering_threshold)
        ...
        # print(best_predecessor_from_source)
        # print(best_successor_to_sink)
        for node in self.nodes:
    ...
    def show_dfg(self, filename="model.png"):
        g = nx.DiGraph()
        for edge in self.edges:
            g.add_edge(edge.src.label, edge.tgt.label)
    ...
    def export_dfg_as_image(self, filename="model.png"):
        g = nx.DiGraph()
        for edge in self.edges:
            g.add_edge(edge.src.label, edge.tgt.label)
        nx.draw(g, with_labels=True)
